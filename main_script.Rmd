...
```
```{r}
str(raw_data_test, give.attr = FALSE)
```
...
data_clean = raw_data_typed %>%
  ...
  replace_na(list(bounces=0,newVisits=0))
...
data_clean = data_clean %>% 
  ...
  mutate(browser=fct_lump_n(browser, n=2),
         operatingSystem=fct_lump_n(operatingSystem, n=4),
         source=fct_lump_lowfreq(source),
         country=fct_lump_n(country, n=4),
         channelGrouping=fct_lump_n(channelGrouping, n=4),
...
browser_levels <- levels(data_clean$browser)
operating_system_levels <- levels(data_clean$operatingSystem)
source_levels <- levels(data_clean$source)
country_levels <- levels(data_clean$country)
device_category_levels <- levels(data_clean$deviceCategory)
...
data_aggregated = data_clean %>%
    ...
    reframe(across(where(is.factor), getMode),
            ...
            timeSinceLastVisit = mean(timeSinceLastVisit), ##not sure about average
...
Perform power transformations on numeric variables:
```{r, echo=F}
pTransf = function(x) {
  p = powerTransform(x ~ 1, family="yjPower")
  return(yjPower(x,p1$lambda))}
...
Model OLS using stepwise selection to reduce # of factor vars
```{r, echo=F}

## downsample to 1/2 of training data to not overload memory
data_sampled = data_aggregated %>% slice_sample(prop=0.50)

train_control <- trainControl(method = "cv", number = 5)

ols_model <- train(log_revenue ~ . - custId, 
                           data = data_sampled, 
                   method = "leapSeq", 
                   tuneGrid = expand.grid(nvmax = 1:16),
...
lasso_model <- train(log_revenue ~ . - custId,
                           data = data_sampled, 
                   method = "lasso", 
...
enet_model <- train(log_revenue ~ . - custId,
                           data = data_sampled, 
                   method = "enet", 
