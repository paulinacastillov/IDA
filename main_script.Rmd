...
top_level_domain_levels <- levels(data_clean$topLevelDomain)
...
data_clean_test = data_clean_test %>% 
  ...
  mutate(browser = factor(browser, levels = browser_levels) %>% fct_na_value_to_level("Other"),
         ...
         topLevelDomain = factor(topLevelDomain, levels = top_level_domain_levels) %>% fct_na_value_to_level("Other"))
```
...
sampled_data = data_aggregated %>% slice_sample(prop=0.5)
...
Lasso model requires low frequency factors to be dropped
```{r, echo=F}

lasso_model <- train(log_revenue ~ channelGrouping + browser + operatingSystem +
                       country + month + visitNumber + timeSinceLastVisit +
                       pageviews + visit_hour, 
