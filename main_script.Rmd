...
Create Linear Discriminant feature on customer buying or not
```{r, echo=F}
data_processed = data_processed %>% mutate(bought = as_factor(ifelse(log_revenue>0,"y","n")))

set.seed(42)
trPos = createDataPartition(data_processed$bought, p=0.75, list=F)
...
data_processed = data_processed %>% 
  mutate(lda1 = (lda_model %>% predict(data_processed) %>% pull()))
data_processed_test = data_processed_test %>% 
  mutate(lda1 = (lda_model %>% predict(data_processed_test) %>% pull()))
```
...
```{r, echo=F}
set.seed(42)
trPos = createDataPartition(data_processed$log_revenue, p=0.75, list=F)
training = data_processed[trPos,]
testing = data_processed[-trPos,]
train_control= trainControl(method = "cv", number = 5)
```
...
tune_list = list(
    ...
    lasso = caretModelSpec(method = "lasso", tuneLength = 16),
    earth = caretModelSpec(method = "earth", tuneLength = 16))
    
models = caretList(
  log_revenue ~ . + channelGrouping:month,
  data = select(training, -c(bought,custId)),
  methodList = c("lm"),
  trControl = train_control,
  tuneList = tune_list,
  metric = "RMSE")
